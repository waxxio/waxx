...
module App::Grp 
  ...
  has(
    ...
    person_id:            {type: "integer",label:"Person ID", is:"person:person.person_id"},
  ...
  def usr(x, usr_name:'', id:0)
    x.db.exec("
      SELECT usr.id, usr.id as person_id, person.company_id, 
      usr_name, password_sha256, salt_aes256, last_login_date, last_login_host, password_mod_date
      FROM usr INNER JOIN person ON usr.id = person.person_id
      WHERE usr_name = $1 
      OR usr.id = $2 
